\ieeesection[DETAILED SOFTWARE DESIGN]{DETAILED SOFTWARE DESIGN}
...
\textbf{PyTorch → ONNX Conversion:} Using the built-in export functionality of Ultralytics, the \texttt{.pt} weights can be converted to the open ONNX intermediate representation format with a single line of code:
...
\label{sec:chassis}
This module combines real-time proximity sensing with a closed-loop PID chassis controller, responsible for executing the final stage of ball retrieval: alignment, approach, and physical ingestion. It connects the visual perception pipeline (Section 3.1.6) with the intake actuator (Section 3.X) through a tightly coupled state machine, simultaneously fusing two independent data streams — the image-plane centroid offset from the YOLO detector, and millimeter-resolution distance readings from the RoboMaster EP's onboard infrared sensor.
