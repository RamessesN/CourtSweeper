...
Velocity commands arriving via \texttt{/cmd\_vel} are converted to per-wheel RPM targets using the Mecanum differential mixing equations (see \S\ref{sec:theory_kinematics} for the kinematic derivation). A scaling factor of 100 maps the normalized Twist velocities to the chassis wheel-speed range:
...
The first phase of autonomous operation constructs a 2D occupancy grid map of the tennis court (theoretical principles in \S\ref{sec:theory_nav}). The mapping pipeline integrates an RPLidar A1M8, the \texttt{slam\_toolbox} ROS2 package, and a custom bridge node.
...
Once the occupancy grid map is saved, the system transitions to autonomous navigation using the \textbf{Nav2} (Navigation2) framework (algorithmic details in \S\ref{sec:theory_nav}). Nav2 provides AMCL-based localization, a global planner, a local controller, and behavior-tree-driven mission execution.
...
The robot's behavioral logic is governed by a two-layer hierarchical Finite State Machine (formal definition in \S\ref{sec:theory_fsm}), illustrated in Figure~\ref{fig:workflow}. The upper layer handles iOS-originated interrupt commands with unconditional priority; the lower layer executes the autonomous mission workflow.
